            ...
            <div class="container-fluid">
                <div class="list_acc row">
                    <div class="col-md-12" style="min-height: 40rem;">
                        <div class="list_admin border rounded-lg">
                            <div class="create-admin bg-info p-3 w-100 d-flex justify-content-between">
                                <h2 class="text-white">Admin Account</h2>
                                <a href="#" class="btn btn-success btn-createAcc py-2 text-white">Create Account</a>
                            </div>
                            <div class="table_admin p-3">
                                        ...
                                        ?>
                                                <tr>
                                                    ...
                                                    <td class="d-flex justify-content-around">
                                                        <a data-id="<?php echo $value['id']; ?>" href="#" class="btn btn-warning btn-updateAdmin text-white">Update</a>
                                                        <button name_admin="<?php echo $value['fullname'] ?>" id_admin="<?php echo $value['id']; ?>" class="btn btn-danger btn-delete">Delete</button>
                                                    </td>
                                                </tr>
                                        <?php
                                            }
                                ...
                                <form id="form_delete" action="deleteAdminAjax.php" method="get">
    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="addNewAdmin" method="post" class="border rouned-lg">
                    <h2 class="text-white bg-info p-3 w-100">Create Account</h2>
                    <div class="input-form p-3">
                        <div class="form-group mb-3">
                            <label>Account For </label>
                            <input type="text" class="form-control mt-1" id="role" name="role" placeholder="Enter role" value="admin" readonly>
                        </div>
                        <div class="form-group mb-3">
...
<div class="modal fade" id="updateModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="updateAdmin" method="post" class="border rouned-lg">
                    <h2 class="text-white bg-info p-3 w-100">Update Admin Account</h2>
                    <input type="hidden" name="id">
                    <div class="input-form p-3">
                        <div class="form-group mb-3">
                            <label>Fullname </label>
...
<script>
    function handleUpdateClick() {
        $('.btn-updateAdmin').on('click', function(e) {
            e.preventDefault();
            let id = $(this).attr('data-id');
            console.log(id);
            $('#updateModal').modal('show');
            $.ajax({
                url: 'getAdminById.php?id=' + id,
                type: 'get',
                contentType: false,
                processData: false,
                success: function(res) {
                    $('#updateAdmin').html(res);
                },
                error: function(err) {
                    console.log(err)
                }
    ...
    function handleDeleteClick() {
        $('.btn-delete').on('click', function(e) {
            e.preventDefault();
            let id_admin = $(this).attr('id_admin');
            let name_admin = $(this).attr('name_admin');
            Swal.fire({
                title: `Are you sure you want to delete student ${name_admin} ?`,
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: 'deleteAdminAjax.php?id=' + id_admin,
                        type: 'GET',
                        contentType: false,
                        processData: false,
                        success: function(res) {
                            ...
                            if (data.success == 'true') {
                                swal.fire({
                                    title: 'Success',
                                    text: 'Delete Admin Account Successfully!',
                                    icon: 'success',
                                    confirmButtonText: "OK"
                                });
                                $('.table_admin table tbody').html(data.html_table);
                                handleUpdateClick();
                                handleDeleteClick();
                    ...
                    Swal.fire(
                        'Not Delete!',
                        'Admin Account has not been deleted yet.',
                        'error'
                    )
                }
            })
    ...
    $(document).ready(function() {
        ...
        $('#addNewAdmin').on('submit', function(e) {
            ...
            if ($('#password').val() == $('#password_confirm').val()) {
                ...
                data.append('addAdmin', 'true');
                $.ajax({
                    url: 'createAccountAdminAjax.php',
                    type: 'POST',
                    data: data,
                    contentType: false,
                    processData: false,
                    success: function(res) {
                        ...
                        if (data[0] == 'success') {
                            ...
                            $('#addNewAdmin')[0].reset();
                            $('.table_admin table tbody').html(data[1]);
                            handleUpdateClick();
                            handleDeleteClick();
                        }
                    },
        ...
        $('#updateAdmin').on('submit', function(e) {
            ...
            let form = e.target;
            let data = new FormData(form);
            data.append('updateAdmin', 'true');
            $.ajax({
                url: 'updateAdminAjax.php',
                type: 'POST',
                data: data,
                contentType: false,
                processData: false,
                success: function(res) {
                    ...
                    if (typeof(data_return) == 'object' && data_return.success == 'true') {
                        swal.fire({
                            title: 'Success',
                            text: 'Update Admin Account Successfully!',
                            icon: 'success',
                            confirmButtonText: "OK"
                        });
                        $('#updateModal').modal('hide');
                        $('.table_admin table tbody').html(data_return.html_table);
                        handleUpdateClick();
                        handleDeleteClick();
                    }
                },
